...
class AddTeamFormModal extends Component
{
    /**
     * @var string
     */
    ...
    public function render()
    {
        ...
        $teamIds = array_merge($teamIds, $user->ownedTeams->pluck('id')->toArray());

        if ($user->hasRole('agent')) {
            $offerForms = count($teamIds) > 0 ? OfferForm::where(function ($query) {
                ...
                ->whereNull('parent_id')
                ->whereHas('teams', function ($query) use ($teamIds, $user) {
                    if ($user->hasRole('agent')) {
        ...
        return view('livewire.offer-forms.add-team-form-modal', compact('offerForms'));
    }
        ...
        if (count($this->selectedForms) > 0) {
            foreach ($this->selectedForms as $selected) {
                ...
                if ($selectForm) {
                    ...
                    $newForm->team_id = optional($selectForm->teams()->first())->id;
                    $newForm->source = 'team-shared';
    ...
    public function destroy() {
        ...
        OfferForm::whereIn('id', $this->selectedForms)->teams()->detach();
