<div class="mt-4 w-full-md-75">
    <div class="form-group">
        <x-input
            ...
            id="first_name_{{$stepSection->id}}"
            :placeholder="$stepSection->placeholder ?? 'Your first name'"
            type="text"
            wire:loading.attr="disabled"
            wire:target="onFormInputChange"
            x-on:change.debounce.500ms="$wire.onFormInputChange('first_name', $el.value);"
            x-init="$el.value = '{{ $this->defaultOrValue }}'"
        />
        <label for="first_name{{$stepSection->id}}" class="animated-label"></label>
    </div>
    @if($routeIsEdit)
        <fieldset
            x-data="{
